...
;; adaptor :: synatx -> syntax
;; Purpose: This is the marco that converts the adaptor syntax into the approperate racket
;;  function. The name of the function is: <name supplied>-adaptor. See examples below for
;;  more details.
(define-syntax (adaptor stx)
  ...
  (syntax-parse stx
    [(_ adaptor-name:id (p:lowc ... <- func:id) ...)
     #:with adapt-fn-name (format-id #'adaptor-name "~a-adaptor" #'adaptor-name)
...
(module+ test
  ...
  (check-equal? (graph-adaptor '((A a B) (B a B))) '("AaB" "BaB") "fsa rule should return AaBBaB")
  (check-equal? (graph-adaptor '((A 1 B) (B a B))) '("SPECIAL A1B" "SPECIAL BaB") "special fsa rule should return AaBHELLO BaB")
  (check-equal? (graph-adaptor '(((A a A) (A a)) ((B b B) (B b)))) '("AaAAa" "BbBBb") "pda rule should return AaAAaBbBBb")
  (check-equal? (graph-adaptor '(((A a) (B b)) ((C c) (D d)))) '("AaBb" "CcDd") "tm rule should return AaBbCcDd"))